...
export class MiyuuDate extends DateOptions {
    ...
    constructor(settings?: DateOptions) {
        super();
        if (!settings) { return; }
        if (settings.locale) { this.setLocale(settings.locale); }
        if (settings.display) { this.setDisplay(settings.display); }
    }
    ...
    setDisplay(display: DateDisplay) {
        if (display.year) { this.display.year = display.year; }
        if (display.month) { this.display.month = display.month; }
        if (display.day) { this.display.day = display.day; }
        if (display.hour) { this.display.hour = display.hour; }
        if (display.minute) { this.display.minute = display.minute }
    }

    getLocaleString(date: Date) {
     ...
     */
    private properLabel(time: number, label: string) {
        if (time === 0) {
            return null;
        }
        if (time === 1) {
